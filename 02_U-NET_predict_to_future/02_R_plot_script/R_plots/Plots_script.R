...
Area_trajectory = 
  ggplot() +
  geom_point(data = Historical_area,aes(x=year,y=area_km2,color='Historical'),size=2) +
  geom_line(data = Project_area,aes(x=year,y=area_km2,color = "Exponential Fit")) +
  ...
  labs(y = bquote('Area ('*10^3 ~km^2*')'),# bquote('Area ('*10^3 ~km^2*')'
       x = 'Year')
...
Train_fig = Train_loss %>% 
  ...
  ggplot() +
  geom_point(aes(x=epoch,y=val_loss),color='grey50',alpha=0.5) +
...
formula = Proj_fractal_dimension ~ True_fractal_dimension
fractal_dimension = landscape_metrics %>% 
  ggplot() +
  geom_point(aes(x=True_fractal_dimension,y=Proj_fractal_dimension)) +
  geom_smooth(aes(x=True_fractal_dimension,y=Proj_fractal_dimension),
              method = 'lm',color='grey30',fill='grey50') +
  geom_abline(slope = 1,intercept = 0,linetype="dashed") +
  geom_text(x = 1.15, y = 1.3, size = 2.5, 
            label = lm_eqn(landscape_metrics,formula), 
            parse = TRUE) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        ...
        axis.line.x.bottom = element_line(),
        axis.line.y.left = element_line()) +
  xlab('Fractal Dimension (real)')+
  ylab('Fractal Dimension (prediction)') 
...
ggsave(plot = Area_trajectory,
       filename = "../Figures/R_Fig_4_Area_trajectory.pdf", 
       width = 10, 
       height = 6, 
       units = "cm",
       dpi=500)
